...
const EMAIL_FROM =
  process.env.EMAIL_FROM?.trim() ?? "WellPage <onboarding@resend.dev>";
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #E4DED4;border-radius:16px;overflow:hidden;">
      <tr>
        <td style="padding:28px 32px;border-bottom:1px solid #EFE9DE;">
          <div style="font-size:12px;letter-spacing:0.18em;color:${BRAND_COLOR};text-transform:uppercase;">WellPage</div>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td style="padding:20px 32px 28px;border-top:1px solid #EFE9DE;color:${MUTED_COLOR};font-size:12px;">
          Você recebeu este email porque se cadastrou no site da WellPage.
        </td>
      </tr>
    </table>
  </body>
...
export async function sendEarlyAccessWelcomeEmail(to: string) {
  const subject = "Bem-vindo ao Acesso Antecipado WellPage";
  const html = layout({
    heading: "Seu lugar no Acesso Antecipado está reservado",
    bodyHtml: `
      <p>Obrigado por querer fazer parte desde o início. A WellPage foi pensada pra nutricionistas e personal trainers que levam o negócio a sério — e a sua escolha de entrar agora ajuda a moldar o produto.</p>
      ...
      <p style="margin-top:24px;color:${MUTED_COLOR};">Até já,<br/>Time WellPage</p>
    `,
  });
  const text = [
...
export async function sendWaitlistWelcomeEmail(to: string) {
  const subject = "Você está na lista de espera do WellPage";
  const html = layout({
    heading: "Obrigado por entrar na lista de espera",
    bodyHtml: `
      <p>Seu email foi confirmado na lista de espera da WellPage.</p>
  ...
  const text = [
    "Obrigado por entrar na lista de espera da WellPage.",
    "",
